...
static char *verif(char *str, int c, int precision)
{
    while (str[my_strlen(str) - 1] == '0' && c + 1 < precision) {
...
static int putall(char *str, int c, int precision)
{
    ...
    str[my_strlen(str) - 1] = '\0';
    str = verif(str, c, precision);
    while (i <= c)
        my_putchar(str[i++]);
    if (c + 1 < my_strlen(str)) {
        my_putchar('.');
        c = -1;
    }
...
static int my_double_f2(double nb, int maj, int precision)
{
    ...
    c = putall(str, c, precision);
    return ((nb2 < 1 && nb2 > 0) ? (my_strlen(str) - 1) :
            (my_strlen(str) + c));
}
